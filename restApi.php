...
//SCRIPT
  //  $tip =$_GET["tip"];
    ...
    if($tip=="zaposleni")
    {
      $queryLinkAndText=mysqli_query($link,"SELECT DISTINCT Z.ID AS ID, Z.Ime AS Ime, Z.Priimek AS Priimek, REPLACE(REPLACE(REPLACE(LOWER(CONCAT(CONCAT(CONCAT(Z.Ime,'.',Z.Priimek)),'@fe.uni-lj.si')),'ž','z'),'č','c'),'š','s') AS Eposta, TZ.Opis AS TipZaposlenega, GU.Besedilo AS GovorilneUre, CONCAT('+3861 4768',TEL.Cifra) AS Telefonska, P.Ime AS Prostor, CL.VlogaZaposlenega AS Vloga, LAB.LabIme AS Lab FROM zaposleni Z INNER JOIN zaposlenje ZJ ON Z.ID=ZJ.IDZaposlenega INNER JOIN lttipzaposlenega TZ ON ZJ.IDTipa=TZ.ID INNER JOIN govorilneure GU ON GU.IDZaposlenega=Z.ID INNER JOIN telefonska TEL ON TEL.IDZaposlenega=Z.ID INNER JOIN zaposlenprostor ZP ON ZP.IDZaposlen=Z.ID INNER JOIN ltprostor P ON ZP.IDProstor = P.ID INNER JOIN clanilaboratorija CL ON CL.IDZaposlenega=Z.ID INNER JOIN laboratorij LAB ON CL.IDLab=LAB.ID ORDER BY ID, Z.Priimek, Z.Ime"
      );
    }
    if($tip=="student")
    {
